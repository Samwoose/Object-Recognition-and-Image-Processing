...
def pixelHop2TransBatchProcessor_Unit1(X,pixelHop2Model):
    ...
    for batchCounter in range(2,numOfBatch+1):
        ...
        crrUpperIndex = int( (batchSize-1) + batchSize*(batchCounter - 1) + 1 )#49 is from batch size 50
...
def featureSelector_binMethod(X,Y):
    ...
    sortedIndex = np.argsort(crossEntropyValStorage) #return indeces that would be the indeces when the array is sorted
    if(sortedIndex.shape[0] >=1000):
        numOfNs = int(1000)
    else:
        numOfNs = int(sortedIndex.shape[0]) #select 33.3 persent of lowest values
...
SaabArgs = [{'num_AC_kernels':-1, 'needBias':False, 'useDC':True, 'batch':None, 'cw':False},
           ...
           {'num_AC_kernels':-1, 'needBias':True, 'useDC':True, 'batch':None, 'cw': True}]
shrinkArgs = [{'func':Shrink, 'win':3 , 'stride':1 ,'depth':1},
             {'func': Shrink, 'win':3, 'stride': 1 ,'depth':2},
             {'func': Shrink, 'win':3, 'stride': 1 ,'depth':3}]
...
f.close()
f = open('Train1562.pckl', 'rb')
XTrain1562, YTrain1562 = pickle.load(f)
f.close()
...
loaded_pixelHop2_6250 = pickle.load(open('pixelHop2_Cla_6250_3x3.sav','rb'))
subXTrain4=XTrain1562
subYTrain4 =YTrain1562
...
#Seperate extracted features from each hop
timerObj.tic()
module1Out1_test = pixelHop2TransBatchProcessor_Unit1(pre_X_test,loaded_pixelHop2_6250)
...
#Seperate extracted features from each hop
timerObj.tic()
module1Out2_test = pixelHop2TransBatchProcessor_Unit2(pre_X_test,loaded_pixelHop2_6250)
...
#Seperate extracted features from each hop
timerObj.tic()
module1Out3_test = pixelHop2TransBatchProcessor_Unit3(pre_X_test,loaded_pixelHop2_6250)
...
#load pre transformed output
#save outputs from module 1
f = open('module1Out1_train1562_3x3', 'rb')
module1Out1_train = pickle.load(f)
f.close()
f = open('module1Out2_train1562_3x3', 'rb')
module1Out2_train = pickle.load(f)
f.close()
f = open('module1Out3_train1562_3x3', 'rb')
module1Out3_train = pickle.load(f)
f.close()
...
#load pre transfromed output
f = open('module1Out1_test1562_3x3', 'rb')
module1Out1_test = pickle.load(f)
f.close()
f = open('module1Out2_test1562_3x3', 'rb')
module1Out2_test = pickle.load(f)
f.close()
f = open('module1Out3_test1562_3x3', 'rb')
module1Out3_test = pickle.load(f)
f.close()
...
f = open('finalFeature_train1562_3x3_new', 'wb')
pickle.dump(finalFeature_train, f)
f.close()
...
f = open('finalFeature_test1562_3x3_new', 'wb')
pickle.dump(finalFeature_test, f)
f.close()
